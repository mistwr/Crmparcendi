...
import { Loader2, Save, Palette, Megaphone, Plus, Trash2, ExternalLink, MonitorSmartphone } from 'lucide-react'
...
export function SiteManager({ settings, campaigns, userId }: { settings: Settings | null; campaigns: Campaign[]; userId: string }) {
  ...
  async function saveBranding() {
    ...
    const { error } = await (supabase.from('parcendi_site_settings') as any).upsert(payload, { onConflict: 'id' })
    setSaving(false)
  ...
  async function addCampaign() {
    ...
    const { data, error } = await (supabase.from('parcendi_site_campaigns') as any).insert({
  ...
  async function toggleCampaign(campaign: Campaign, field: 'is_active' | 'is_featured') {
    ...
    const { error } = await (supabase.from('parcendi_site_campaigns') as any).update({ [field]: next, updated_at: new Date().toISOString() }).eq('id', campaign.id)
  ...
  async function deleteCampaign(id: string) {
    ...
    const { error } = await (supabase.from('parcendi_site_campaigns') as any).delete().eq('id', id)
  ...
  return (
    <Tabs defaultValue="branding" className="space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {([
                ...
                ['secondary_color', 'Cor escura'],
                ['accent_color', 'Cor de destaque'],
              ] as const).map(([key, label]) => (
                <div key={key} className="space-y-1.5">
                  <Label>{label}</Label>
                  <div className="flex gap-2">
                    <input type="color" value={form[key]} onChange={(e) => set(key, e.target.value)} className="h-10 w-12 cursor-pointer rounded border border-border bg-card p-1" />
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <div className="p-5 text-white" style={previewStyle}>
              ...
              <button type="button" className="mt-5 rounded-lg bg-white px-4 py-2 text-xs font-bold" style={{ color: form.primary_color }}>
                {form.consultation_cta_label}
              </button>
            </div>
            <div className="p-4 text-xs text-muted-foreground">Pré-visualização rápida das cores e do hero. As alterações só ficam gravadas ao clicar em Guardar.</div>
          </div>
        </div>
        <div className="flex justify-end">
          <Button onClick={saveBranding} disabled={saving} className="gap-2 bg-brand text-white hover:bg-brand-dark">
      ...
      <TabsContent value="conteudo" className="space-y-5">
        <div className="rounded-xl border border-border bg-card p-4 sm:p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={form.hero_subtitle ?? ''} onChange={(e) => set('hero_subtitle', e.target.value)} rows={3} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label>Texto do botão de consultoria</Label>
                <Input value={form.consultation_cta_label} onChange={(e) => set('consultation_cta_label', e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label>URL do simulador de energia</Label>
                <Input value={form.energy_simulator_url ?? ''} onChange={(e) => set('energy_simulator_url', e.target.value)} placeholder="https://..." />
              </div>
            </div>
          </div>
        </div>
        <div className="flex justify-end">
          <Button onClick={saveBranding} disabled={saving} className="gap-2 bg-brand text-white hover:bg-brand-dark">
            {saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Guardar conteúdo
          </Button>
        </div>
      </TabsContent>
      ...
      <TabsContent value="campanhas" className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold">Campanhas visíveis no site</h2>
            <p className="text-xs text-muted-foreground">Ativa, destaca ou cria campanhas sem alterar o código.</p>
          </div>
          <Button onClick={() => setShowNew((v) => !v)} className="gap-2 bg-brand text-white hover:bg-brand-dark"><Plus size={15} /> Nova campanha</Button>
        </div>
          ...
          <div className="rounded-xl border border-brand/30 bg-card p-4 sm:p-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={campaignDraft.segment} onChange={(e) => setCampaignDraft({ ...campaignDraft, segment: e.target.value })}>
                  <option value="telecom">Telecom</option><option value="energia">Energia</option><option value="credito">Crédito</option><option value="imobiliario">Imobiliário</option><option value="seguros">Seguros</option>
                </select>
              </div>
        ...
        <div className="grid gap-3">
          {items.length === 0 && <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Ainda não existem campanhas configuradas para o site.</div>}
          {items.map((campaign) => (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {campaign.operator && <Badge variant="outline">{campaign.operator}</Badge>}
                    {campaign.is_featured && <Badge>Destaque</Badge>}
                  </div>
                  {campaign.subtitle && <p className="mt-1 text-sm text-muted-foreground">{campaign.subtitle}</p>}
                  {campaign.price != null && <p className="mt-2 text-lg font-bold">{Number(campaign.price).toFixed(2)}€ <span className="text-xs font-normal text-muted-foreground">{campaign.price_suffix}</span></p>}
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" onClick={() => toggleCampaign(campaign, 'is_active')}>{campaign.is_active ? 'Desativar' : 'Ativar'}</Button>
                  <Button size="sm" variant="outline" onClick={() => toggleCampaign(campaign, 'is_featured')}>{campaign.is_featured ? 'Retirar destaque' : 'Destacar'}</Button>
                  {campaign.cta_url && <Button size="icon" variant="outline" asChild><a href={campaign.cta_url} target="_blank" rel="noreferrer"><ExternalLink size={14} /></a></Button>}
                  <Button size="icon" variant="outline" onClick={() => deleteCampaign(campaign.id)} className="text-red-500"><Trash2 size={14} /></Button>
                </div>
              </div>
            </div>
          ))}
